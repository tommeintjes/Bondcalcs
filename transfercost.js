...
function getDeedsAmt(totamt) {
    ...
    else if ((totamt > 600000)  && (totamt <= 800000))      {   deedsamt = 700; }
    else if ((totamt > 600000)  && (totamt <= 800000))      {   deedsamt = 800; }
...
function calcTrans(purchaseP,bondAmount){
    ...
    if (purchaseP <= 600000) {
       ...
       transDutyN = 12000 + ((purchaseP - 1000000) * 0.05);
    } else if ( purchaseP > 1500000){
       transDutyN = 37000 + ((purchaseP - 1500000) * 0.08);
    }
    ...
    if (convOrSect=="conventional"){
        ...
        excluUseN   = 0;
        proRataN    = 4000;
        office_search = 125.40;
    } else if ( convOrSect=="sectional"){
        ratesN      = 1050;
        excluUseN   = (1000 + ( 1000 * 0.14)) * useExclu;
        proRataN    = 5000;
        office_search = 200;
    }
    ...
    totalN3 = totalN2 + (postageN + ratesN + proRataN + excluUseN + office_search+electronic_fee) ;

    //Return values to html

    //alert('trans: '+formatCalc(transFeeN));
    document.getElementById("transFeeN").innerHTML      = "R " + formatCalc(transFeeN);

    document.getElementById('vatN').innerHTML           = "R " + formatCalc(vatN);

    document.getElementById('deedOfficeN').innerHTML    = "R " + formatCalc(deedOfficeN);

    document.getElementById('totalN1').innerHTML        = "R " + formatCalc(totalN1);

    document.getElementById('transDutyN').innerHTML     = "R " + formatCalc(transDutyN);

    document.getElementById('totalN2').innerHTML        = "R " + formatCalc(totalN2);

    document.getElementById('postageN').innerHTML       = "R " + formatCalc(postageN);

    document.getElementById('ratesN').innerHTML         = "R " + formatCalc(ratesN);

    document.getElementById('prorataN').innerHTML       = "R " + formatCalc(proRataN);

    document.getElementById('excluN').innerHTML         = "R " + formatCalc(excluUseN);

    document.getElementById('deedOfficeSearchN').innerHTML  = "R " + formatCalc(office_search);

    document.getElementById('electroDocN').innerHTML    = "R " + formatCalc(electronic_fee);

    document.getElementById('totalN3').innerHTML        = "R " + formatCalc(totalN3);
...
function calcBond(bondAmount) {
    ...
    if (bondAmount>0) {
        ...
        */
        ...
        if(bondAmount) {
            ...
            total2 = eval(fees + vat + deeds + insCert + postage_petties_bond + bcf + docfee).toFixed(2);
    ...
    document.getElementById('fees').innerHTML             = "R " + formatCalc(fees);
    document.getElementById('vat').innerHTML              = "R " + formatCalc(vat);
    document.getElementById('deeds').innerHTML            = "R " + formatCalc(deeds);
    document.getElementById('postagePettiesBC').innerHTML = "R " + formatCalc(postage_petties_bond);
    document.getElementById('insCert').innerHTML          = "R " + formatCalc(insCert);
    document.getElementById('bcf').innerHTML              = "R " + formatCalc(bcf);
    document.getElementById('docfee').innerHTML           = "R " + formatCalc(docfee);
    document.getElementById('tot2').innerHTML             = "R " + formatCalc(total2);
}
